...
from data.create_graph import get_graph
...
def train_fn(
    ...
    if embedding_module_type == "categorical":
        ...
        embedding_module: EmbeddingModule = LocalEmbeddingModule(
            ...
            dataset_text_path=dataset_text_path,
            adj=adj,
        )
    else:
    ...
    if sampling_strategy == "in-batch":
        ...
        negatives_sampler = LocalNegativesSampler(
            num_items=dataset.max_item_id,
            fusion_emb=model._embedding_module.ELSM,
    ...
    for epoch in range(num_epochs):
        ...
        for row in iter(train_data_loader):
            ...
            opt.zero_grad()
            input_embeddings = model.module.get_item_embeddings_forward(seq_features.past_ids)
            ...
            loss = model.module._embedding_module.contrastive_loss() + ar_loss(
        ...
        for eval_iter, row in enumerate(iter(train_data_loader)):
            seq_features, target_ids, target_ratings = movielens_seq_features_from_row(row, device=device,
                                                                                       max_output_length=gr_output_length + 1)
        ...
        logging.info(f"VAL rank {rank}: eval @ epoch {epoch} in {time.time() - eval_start_time:.2f}s: "
                     f"NDCG@10 {ndcg_10:.4f}, NDCG@50 {ndcg_50:.4f}, HR@10 {hr_10:.4f}, HR@50 {hr_50:.4f}, MRR {mrr:.4f}")
        last_training_time = time.time()

        early_stop(np.array(mrr))
        ...
        if best_val_mrr < mrr:
            ...
            logging.info(f"TEST rank {rank}: eval @ epoch {epoch} in {time.time() - eval_start_time:.2f}s: "
                         f"NDCG@10 {ndcg_10:.4f}, NDCG@50 {ndcg_50:.4f}, HR@10 {hr_10:.4f}, HR@50 {hr_50:.4f}, MRR {mrr:.4f}")

            best_ndcg_10 = ndcg_10
            best_ndcg_50 = ndcg_50
            best_hr_10 = hr_10
            best_hr_50 = hr_50
            best_mrr = mrr
        ...
        if early_stop.early_stop or epoch == (num_epochs - 1):
            logging.info(
                f"NDCG@10 {best_ndcg_10:.4f}, NDCG@50 {best_ndcg_50:.4f}, HR@10 {best_hr_10:.4f}, HR@50 {best_hr_50:.4f}, MRR {best_mrr:.4f}")
