...
class DataProcessor:
    ...
    def output_mapping_csv(self) -> str:
        return f"tmp/{self._prefix}/id_mapping.csv"
...
class AmazonDataProcessor(DataProcessor):
    ...
    def preprocess_rating(self) -> int:
        ...
        item_id_count['item_count'] = item_id_count.apply(
            lambda x: x['item_count'] if x['unique_values'] in items_ else 0, axis=1)
        ...
        ratings = ratings.drop(columns=['item_count', 'user_count'])

        item_num = len(item_id_count.set_index("unique_values"))
        user_num = len(user_id_count.set_index("unique_values"))
        del item_id_count
        del user_id_count

        while True:
            item_id_count = ratings['item_id'].value_counts().rename_axis('unique_values').reset_index(
                name='item_count')
            user_id_count = ratings['user_id'].value_counts().rename_axis('unique_values').reset_index(
                name='user_count')
            ratings = ratings.join(item_id_count.set_index("unique_values"), on="item_id")
            ratings = ratings.join(user_id_count.set_index("unique_values"), on="user_id")
            ratings = ratings[ratings['item_count'] >= 5]
            ratings = ratings[ratings['user_count'] >= 5]
            if (len(item_id_count.set_index("unique_values")) == item_num) and (len(user_id_count.set_index("unique_values")) == user_num):
                break
            ratings = ratings.drop(columns=['item_count', 'user_count'])
        ...
        categories = ratings["item_id"].cat.categories
        item_id_mapping = {category: index for index, category in enumerate(categories)}
        item_id_mapping_df = pd.DataFrame(list(item_id_mapping.items()), columns=['item_id', 'map_id'])
        ...
        item_id_mapping_df.sample(frac=1).reset_index().to_csv(self.output_mapping_csv(), index=False, sep=",")
...
def get_common_preprocessors():
    amzn_pantry_dp = AmazonDataProcessor(
        download_path=None,
        saved_name="tmp/Prime_Pantry.csv",
        meta_name="tmp/meta_Prime_Pantry.json.gz",
        prefix="amzn_pantry",
        expected_num_unique_items=4898,
    )
    amzn_scientific_dp = AmazonDataProcessor(
        download_path=None,
        saved_name="tmp/Industrial_and_Scientific.csv",
        meta_name="tmp/meta_Industrial_and_Scientific.json.gz",
        prefix="amzn_scientific",
        expected_num_unique_items=4385,
    )

    return  {"amzn-pantry": amzn_pantry_dp,\
            "amzn-scientific": amzn_scientific_dp}